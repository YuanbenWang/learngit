...
if __name__=="__main__":
    ...
    for k in range(100,1100,100):
        model1 = SelectKBest(chi2, k=1000)
        train_new = model1.fit_transform(train, label)
        predictNB = predict_NB(train_new, label)
        predictSVM = predict_SVM(train_new, label)
        predictRF = predict_RF(train_new, label)
        acc.append([predictNB,predictSVM,predictRF])
